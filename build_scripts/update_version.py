...
parser.add_argument('-f', '--full', action='store', dest='full', type=str, default=False,
                    help="Change the full version number X.X.X", metavar="VERSION")
parser.add_argument('+M', '--major', action='store', dest='major', nargs='?', type=int, default=False, const=True,
                    help="Increase the major version number X.0.0", metavar="MAJOR")
parser.add_argument('+m', '--minor', action='store', dest='minor', nargs='?', type=int, default=False, const=True,
                    help="Increase the minor version number -.X.0", metavar="MINOR")
parser.add_argument('+b', '--bug', action='store', dest='bug', nargs='?', type=int, default=False, const=True,
                    help="Increase the minor version number -.-.X", metavar="BUG")
parser.add_argument('-m', '--message', action='store', dest='message', type=str, nargs='*', default=[''],
                    help="The message to be added to changelog.txt. If no --message given, do not add to changelog.txt",
                    metavar="MESSAGE")
...
# Validate inputs
nArgs = sum([args.full is not False, args.major is not False, args.minor is not False, args.bug is not False])
...
if (nArgs == 1):
    ...
    elif (args.major is not False):
        if (args.major is True):
            newVersion = [currVersion[0]+args.major, currVersion[1], 0]
        else:
            newVersion = [args.major, currVersion[1], 0]
    elif (args.minor is not False):
        if (args.minor is True):
            newVersion = [currVersion[0], currVersion[1]+args.minor, 0]
        else:
            newVersion = [currVersion[0], args.minor, 0]
    elif (args.bug is not False):
        if (args.bug is True):
            newVersion = [currVersion[0], currVersion[1], currVersion[2]+args.bug]
        else:
            newVersion = [currVersion[0], currVersion[1], args.bug]
